...
function formatMoveList(nodes) {
  // nodes = variant path excluding root
...
// ─── Completion screen ────────────────────────────────────────────────────
function CompletionScreen({ opening, variantNodes, variantName, score, onRetry, router }) {
  const total = variantNodes.length - 1;
  const accuracy = total > 0 ? Math.round((score.correct / (score.correct + score.wrong)) * 100) : 100;
    ...
    <div className="completion">
      <div className="completion-inner">
        ...
        <div className="completion-actions">
          ...
          <Link href={`/opening/${opening.id}`} className="btn-ghost">Back to Opening</Link>
          <Link href="/" className="btn-ghost">Repertoire</Link>
        </div>
      </div>
...
function PracticeContent() {
  ...
  const [variantNodes, setVariantNodes] = useState([]); // full path: [root, node1, node2, ...]
  const [variantName, setVariantName] = useState('');
  const [step, setStep] = useState(0); // index into variantNodes for the CURRENT position
  const [status, setStatus] = useState('loading'); // 'loading' | 'player' | 'opponent' | 'wrong' | 'complete'
  ...
  // Load opening + parse variant path
  useEffect(() => {
    const pathStr = searchParams.get('path');
    if (!pathStr) { router.push(`/opening/${params.id}`); return; }
    ...
    getOpening(params.id).then((op) => {
      ...
      if (nodes.length < 2) { router.push(`/opening/${params.id}`); return; }
  ...
  // Determine if current position needs player or auto-play
  const currentNode = variantNodes[step] ?? null;
  const nextNode = variantNodes[step + 1] ?? null;

  function isPlayersTurn() {
    if (!currentNode || !opening) return false;
    const sideToMove = currentNode.fen.split(' ')[1]; // 'w' or 'b'
    return sideToMove === opening.color[0]; // opening.color = 'white'|'black'
  }
  ...
  // Kick off after load (allow status='loading' to settle first)
  useEffect(() => {
  ...
  const handlePieceDrop = useCallback(
    async (sourceSquare, targetSquare) => {
      ...
      if (result.san === nextNode.san) {
        ...
        // Wrong move — reject the drop, show feedback
        setWrongMove(result.san);
        setStatus('wrong');
        setScore((s) => ({ ...s, wrong: s.wrong + 1 }));
        return false; // board reverts to current position
      }
    },
    [status, currentNode, nextNode]
  );

  // ── Square click: show legal moves + click-to-move (player's turn only) ──
  ...
  function handleRetry() {
    setIsRetry((r) => !r); // toggle to re-trigger useEffect
    setStep(0);
    setStatus('loading');
    setScore({ correct: 0, wrong: 0 });
    setWrongMove('');
  ...
  const totalMoves = variantNodes.length - 1; // exclude root
  const completedMoves = Math.max(0, step); // how many moves have been made
    ...
    <div className="practice-page">
      ...
      <div className="topbar">
        <div className="topbar-left">
          <Link href={`/opening/${opening.id}`} className="back-btn">← {opening.name}</Link>
      ...
      <div className="body">
        ...
        <div className="sidebar">
          ...
          <div className="status-panel">
              ...
              <>
                ...
                {showHint && nextNode && (
                  <div className="hint-box">
                    Hint: <strong>{nextNode.san}</strong>
                  </div>
                )}
            ...
            {status === 'opponent' && (
              <>
                <div className="status-icon opp-icon">
                  {opening.color === 'white' ? '♟' : '♙'}
                </div>
      ...
      <style jsx>{`
        ...
        .btn-retry {
          ...
          border: 1px solid rgba(109,186,124,0.3);
          color: #6dba7c;
          padding: 6px 16px;
          font-family: var(--font-body);
          font-size: 12px;
        ...
        .score-correct-badge, .score-wrong-badge {
          font-family: var(--font-mono);
          font-size: 13px;
          font-weight: 700;
          padding: 4px 10px;
